...
import {
  ...
  isValidMWBSchedule,
  isValidWSchedule,
} from "./html_validation.js";
import { jw_epub_parser } from "../browser/utils.browser.js";

export const extractEPUBFiles = async (
  data: string | ArrayBuffer | Buffer | Blob
) => {
  const appZip = new JSZip();

  const contents = await appZip.loadAsync(data as any);
  return contents;
};

export const validateEPUBContents = async (
  data: string | ArrayBuffer | Buffer | Blob
) => {
  const MAX_FILES = 300;
...
export const getHTMLDocs = async (zip: JSZip, isMWB: boolean, isW: boolean) => {
  ...
  for (let [filename] of Object.entries(zip.files)) {
    ...
    if (isValidFile) {
      ...
      const isValidSchedule = isMWB
        ? isValidMWBSchedule(htmlDoc)
        : isW
          ? isValidWSchedule(htmlDoc)
          : false;
...
export const getHTMLWTArticleDoc = async (
  zip: JSZip,
